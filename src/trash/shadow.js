...
function onDepthChange() {
  // Previews
  amount.textContent = depth + 1;
  previewAndroid.style.boxShadow = `
  ...
  // Code
  height = y;
  opacity = interpolate(depth, 1, 24, 0.2, 0.6).toFixed(2);
  radius = interpolate(s.blur, 1, 38, 1, 16).toFixed(2); //(s.blur * 0.45).toFixed(2);
  elevation = depth + 1;
}
